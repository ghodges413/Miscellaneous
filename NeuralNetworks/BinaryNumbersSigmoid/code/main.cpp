...
struct NeuronLayer_t {
	...
	static VecN ActivatePerceptron( NeuronLayer_t & layer, const VecN & activationsIn );
	static VecN ActivateSigmoid( NeuronLayer_t & layer, const VecN & activationsIn );
	static VecN ActivateReLU( NeuronLayer_t & layer, const VecN & activationsIn );
...
/*
====================================================
NeuronLayer_t::ActivatePerceptron
====================================================
*/
VecN NeuronLayer_t::ActivatePerceptron( NeuronLayer_t & layer, const VecN & activationsIn ) {
	VecN z = layer.bias;
...
/*
====================================================
NeuronLayer_t::ActivateSigmoid
====================================================
*/
VecN NeuronLayer_t::ActivateSigmoid( NeuronLayer_t & layer, const VecN & activationsIn ) {
	VecN z = layer.bias;
	z += layer.weights * activationsIn;
	layer.cachedZ = z;
...
====================================================
*/
VecN SigmoidDerivative( const VecN & z ) {
	VecN dSigma( z.N );

	for ( int i = 0; i < z.N; i++ ) {
		const float expz = expf( -z[ i ] );
		const float expz2 = 1.0f + expz;

		dSigma[ i ] = expz / ( expz2 * expz2 );
	}
...
VecN AbsoluteError( const VecN & activations, const VecN & z, const VecN & correctOutput ) {
	...
	VecN dSigma = SigmoidDerivative( z );
...
void TestBinaryNetwork() {
	...
	for ( int epoch = 0; epoch < maxEpochs; epoch++ ) {
		...
		outputActivations = NeuronLayer_t::ActivateSigmoid( outputLayer, inputActivation );
		if ( doPrint ) {
...
VecN FeedForward( NeuronLayer_t * layers, const int numLayers, const VecN & inputs ) {
	...
	for ( int i = 0; i < numLayers; i++ ) {
		activations = NeuronLayer_t::ActivateSigmoid( layers[ i ], activations );
	}
